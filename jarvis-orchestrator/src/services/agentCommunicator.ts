...
export class AgentCommunicator {
   ...
   */
  public async processWithAgents(
  ...
  private async callAgent(
    ...
    try {
      const response = await axios.post(
        `${this.agentServiceUrl}/api/v1/process`,
        payload,
        {
  ...
  private async processFallback(request: OrchestrationRequest, requestId: string): Promise<AgentResult | null> {
    try {
   ...
   */
  public async checkAgentServiceHealth(): Promise<boolean> {
    try {
      const response = await axios.get(`${this.agentServiceUrl}/api/v1/health`, {
        timeout: 5000
      });
   ...
   */
  public async getAvailableAgentTypes(): Promise<string[]> {
    try {
